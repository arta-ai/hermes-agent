...
MEMORY_GUIDANCE = (
    ...
    "'submitted PR Y', 'Phase N done', file counts, or other temporary outcome snapshots. "
    "Age is a signal, not a universal cutoff: classify facts as durable, reviewed/pointer-bound, "
    "TTL/current, or session-only. A useful slow-changing environment fact may be retained "
    "with its source and review condition; operational status belongs in a live source or TTL lane. "
    "If you've discovered a new way to do something, solved a problem that could be "
    "necessary later, save it as a skill with the skill tool.\n"
    "Use one owning lane rather than automatically copying a fact to every memory backend. "
    "Each additional copy needs a distinct consumer and readback purpose.\n"
...
SKILLS_GUIDANCE = (
    "After completing a genuinely reusable non-trivial workflow, fixing a tricky error "
    "whose solution is likely to recur, or discovering a repeatable procedure, save the approach as a "
    "skill with skill_manage so you can reuse it next time.\n"
    "Tool-call count alone is not a reason to create a skill; avoid one-off skill clutter.\n"
