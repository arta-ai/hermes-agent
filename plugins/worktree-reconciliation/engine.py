...
EXPIRY_SCHEMA = "loaw.worktree_lease_expiry.v1"
...
def _retire_expired_clean_worktree_leases(
    state_root: Path,
    repo: Path,
) -> list[dict[str, Any]]:
    active = _active_worktree_leases(state_root, repo)
    if not active or any(not _is_expired(lease) for lease in active):
        return active
    gate = _gate()
    ...
    for listed in active:
        lease_path = Path(str(listed["lease_path"]))
        with _lease_lock(lease_path):
            lease = _load_json(lease_path)
            if lease.get("state") != "ACTIVE" or not _is_expired(lease):
                continue
            event = {
                ...
                "previous_deadline": lease.get("lease_deadline"),
            }
            event_path = _event_path(Path(lease["run_directory"]), "expired-clean")
    ...
    return _active_worktree_leases(state_root, repo)
...
def open_lease(
    *,
    ...
    with _lease_lock(_worktree_lock_path(root, repo_root)):
        active = _retire_expired_clean_worktree_leases(root, repo_root)
        if active:
            existing = active[0]
            violations = [
...
def checkpoint_lease(
    *,
    ...
    gate = _gate()
    root = _state_root(state_root)
    lease_path, initial_lease = load_lease(session_id, run_id, state_root=state_root)
    repo_for_lock = gate.repo_root(initial_lease["worktree"])
    with contextlib.ExitStack() as locks:
        locks.enter_context(_lease_lock(_worktree_lock_path(root, repo_for_lock)))
        ...
        repo = gate.repo_root(lease["worktree"])
        if repo.resolve() != repo_for_lock.resolve():
            raise ReconciliationError("lease worktree changed while acquiring checkpoint locks")
        opened = _load_json(Path(lease["open_receipt"]))
        effective_open = dict(opened)
